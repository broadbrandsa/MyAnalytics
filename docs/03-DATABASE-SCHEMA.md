...
All tables in `public` unless noted. Helper functions in a `private` schema (not exposed via PostgREST). Enable RLS on every table. Migrations via `supabase migration new` / SQL files in `supabase/migrations/`.

## Entity overview

```
clients ─< memberships >─ auth.users (profiles)
clients ─< data_sources >─ oauth_credentials
data_sources ─< sync_runs
data_sources ─< metrics_ga4_daily / metrics_gsc_daily / metrics_gads_daily / metrics_meta_daily
             ─< metrics_gsc_queries / metrics_gsc_pages / dim tables
clients ─< dashboard_configs
```
...
-- source is deliberately text+check, not an enum: PRD requires new sources addable without type migrations
-- (to add a source later: drop+recreate the check constraint)
...
Typed columns over jsonb blobs — chart queries need them. One table per source; long-tail dimensions get their own tables. All money in **micros is converted at write time** to numeric (Ads `cost_micros`/1e6; Meta spend is already currency units).
...
Indexes for dashboard range scans (add for each metrics table):
...
Volume estimate: 50 clients × 4 sources × ~30 rows/day ≈ 200k rows/year — no partitioning needed; revisit past ~20M rows.
...
Rules:
- Sync workers write with the **service-role key** (bypasses RLS) — server-side only.
- **`/api/refresh` role split:** the route authenticates the caller with their session (membership check), then uses the **service-role client** for the writes the flow needs (`clients.last_refresh_at` update, `sync_runs` inserts). Client viewers read `sync_runs` status through RLS (`client_id` policy above) — no write policies for `client_viewer` anywhere.
- `vault.decrypted_secrets` is never selectable by `authenticated`/`anon` roles; only accessed via service-role.
- Index every column used in a policy (`memberships.user_id`, `client_id` everywhere) — done above.

## Vault usage

```sql
-- store (from server code via rpc or direct service-role query):
select vault.create_secret('<refresh_token>', 'google-agency', 'Google agency refresh token');
-- read (service-role only):
select decrypted_secret from vault.decrypted_secrets where id = $1;
-- rotate (Meta 60-day refresh):
select vault.update_secret($1, '<new_token>');
```
